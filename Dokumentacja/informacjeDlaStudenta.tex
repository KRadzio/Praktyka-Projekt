...
\newline
W \textbf{Algorithms.hpp} znajdują się wszyskie deklaracje funkcji.
\newline
W \textbf{Algorithms.hpp} znajduje się również definicja struktury z prarametrami do algorytmów (niektóre nie wymagają parametrów).
...
\newline
Zawiera takie przydatne metody metody jak:
\begin{itemize}
    ...
    \item \textbf{void} RefreshPixelValuesArrays() - odświerz tablice z historamami i dystrybuantami (po poprawnym zakończeniu wątku z algorytmem wątek główny automatycznie odświerzy)
    ...
    \item \textbf{void} SetPixelBlack(int x, int y) - istawia pixel w danym punkcie na kolor czarny
    \item \textbf{void} CopyOnlySurfaceAndSize(Image other) - kopiuje tylko surface i wymiary (użyteczne w algorytmach w których wartość jendego piksela zależy od wartości jego sąsiadów)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{bool} IsThreadRunning() - czy wątek przetwarzający ma się dalej wykonywać (używane przy przerwaniu wykonywnia wątku)
    \item \textbf{void} ThreadStopped() - ustwaia że wątek przetwarzający zakończył wykonywanie
\end{itemize}
